...
const FilterModal = (props: {
  ...
  return (
    ...
    >
      <div
        className="bg-trasparent h-[60%] w-full z-40 cursor-pointer "
        onClick={onClickCloseFilter}
      ></div>
      <div
        className={`
         bg-white h-[40%] flex flex-col gap-7 w-[98vw] rounded-t-xl p-6 shadow-lg shadow-secondColor z-50`}
      >
        <div className="w-full flex justify-between text-mobileTitle">
          ...
          <h3
            className="hover:text-secondColor cursor-pointer duration-300"
            onClick={onClickCloseFilter}
          >
            X
          </h3>
        ...
        <Filter display="flex self-center" width="w-[75%]" gap="gap-[10px] " />
      </div>
    </div>
  );
};
